...
class ReservationApiIntegrationTest {
    ...
    void shouldCreateConfirmAndCheckInReservationAcrossPublicAndStaffApis() throws Exception {
        ...
        String customerAccessToken = registerCustomerAndGetAccessToken("reservation-checkin");
    ...
    @Test
    void shouldExposeTableQrForCompletedPublicReservationWhenAssignedTableHasActiveQr() throws Exception {
        DiningTable diningTable = seedDiningTable();
        tableQrService.generate(new GenerateTableQrRequest(diningTable.getId(), "Reservation QR", null));
        String reservationTime = Instant.now().plusSeconds(7_200).toString();
        String customerAccessToken = registerCustomerAndGetAccessToken("reservation-qr");
        ...
        mockMvc.perform(post("/api/reservations/{reservationId}/confirm", reservationId)
                        ...
                        .content("""
                                {
                                  \"internalNote\": \"Confirmed for QR handoff\"
                                }
                                """))
                .andExpect(status().isOk());
        ...
        mockMvc.perform(post("/api/reservations/{reservationId}/check-in", reservationId)
                        ...
                        .content("""
                                {
                                  \"diningTableId\": %d,
                                  \"internalNote\": \"Seated at QR-enabled table\"
                                }
                                """.formatted(diningTable.getId())))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.status").value("CHECKED_IN"));

        mockMvc.perform(post("/api/reservations/{reservationId}/complete", reservationId)
                        .header(HttpHeaders.AUTHORIZATION, "Bearer " + accessToken))
                .andExpect(status().isOk())
        ...
        mockMvc.perform(get("/api/public/reservations/{reservationCode}", reservationCode))
                ...
                .andExpect(jsonPath("$.status").value("COMPLETED"))
                .andExpect(jsonPath("$.assignedTableId").value(diningTable.getId()))
                .andExpect(jsonPath("$.tableQr.token").isNotEmpty())
                .andExpect(jsonPath("$.tableQr.label").value("Reservation QR"))
                .andExpect(jsonPath("$.tableQr.landingUrl").value(org.hamcrest.Matchers.containsString("/qr/")));
    }
    ...
    void shouldKeepInternalNotesPrivateWhenGuestCancelsReservation() throws Exception {
        ...
        String customerAccessToken = registerCustomerAndGetAccessToken("reservation-cancel");
    ...
    private String registerCustomerAndGetAccessToken(String prefix) throws Exception {
        ...
        MvcResult registerResult = mockMvc.perform(post("/api/public/auth/register/customer")
                        ...
                        .content("""
                                ...
                                """.formatted(suffix, suffix, suffix, phone)))
                .andExpect(status().isOk())
                .andReturn();

        JsonNode registerJson = objectMapper.readTree(registerResult.getResponse().getContentAsString());
        return registerJson.path("accessToken").asText();
    }
