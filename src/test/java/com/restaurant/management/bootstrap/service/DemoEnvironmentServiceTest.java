...
class DemoEnvironmentServiceTest {
    ...
    @Test
    void shouldSeedMinimumDemoDataAndCreateQrOnce() {
        ...
        DemoEnvironmentService service = new DemoEnvironmentService(
                areaRepository,
        ...
        when(tableQrService.generate(any())).thenReturn(new TableQrResponse(
                10L,
                2L,
                "T-01",
                "demo-token",
    ...
    void shouldReuseExistingQrWithoutRegeneratingIt() {
        ...
        tableQr.setLabel("Old label");
        tableQr.setImagePath("qr-codes/demo-token.png");
        tableQr.setActive(false);
        ...
        when(tableQrService.getByTable(2L)).thenReturn(new TableQrResponse(
                5L,
                2L,
                "T-01",
                "demo-token",
        ...
        when(tableQrRepository.save(any(TableQr.class))).thenAnswer(invocation -> invocation.getArgument(0));

        service.ensureBaseline();

        verify(tableQrService, never()).generate(any());
        verify(tableQrService).getByTable(2L);
        verify(tableQrRepository).save(tableQr);
        assertThat(tableQr.isActive()).isTrue();
        assertThat(tableQr.getLabel()).isEqualTo("Demo Table 01");
        assertThat(tableQr.getExpiresAt()).isNull();
    }
}
