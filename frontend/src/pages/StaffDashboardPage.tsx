...
export function StaffDashboardPage({
  ...
  const [isWorkflowDrawerOpen, setWorkflowDrawerOpen] = useState(false);
  ...
  return (
      ...
      headerActions={
        canOpenWorkflow ? (
          <button className="button-secondary py-2.5" onClick={() => openWorkflowDrawer(operationsTab)} type="button">
            <span className="material-symbols-outlined text-lg">right_panel_open</span>
            {t('Operations dock')}
          </button>
        ) : null
      }
    ...
    >
        ...
        <div className="space-y-5">
          <div className="rounded-lg border border-ink/10 bg-white px-5 py-4">
            <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
              <div>
                <p className="text-xs font-bold uppercase tracking-[0.22em] text-slate">{t('Deep workflow access')}</p>
                <p className="mt-2 text-sm leading-7 text-slate">
                  {t('The dashboard above is for scanning. This dock is where the team executes the detailed workflow without leaving the page.')}
                </p>
              </div>
              <button className="button-primary" onClick={() => openWorkflowDrawer(operationsTab)} type="button">
                <span className="material-symbols-outlined text-lg">right_panel_open</span>
                {t('Operations dock')}
              </button>
            </div>
      ...
      <RightDrawer
        ...
        subtitle={t('The workflow drawer keeps detailed actions nearby without pushing the dashboard into a long, heavy page.')}
