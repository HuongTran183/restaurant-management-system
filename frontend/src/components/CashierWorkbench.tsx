...
type CashierWorkbenchProps = {
  ...
  onCreateInvoice: (orderId: number) => void;
  ...
  useEffect(() => {
    ...
    setPaymentForm((current) => {
      ...
      return {
        ...
        amount: currentInvoiceId === selectedInvoice.id && current.amount ? current.amount : remaining.toFixed(2),
      };
    });
  }, [openInvoices]);
      ...
      <section className="space-y-4 rounded-[28px] border border-ink/10 bg-white/65 p-5">
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div>
            ...
            <h3 className="mt-2 font-display text-2xl text-ink">Confirm, cancel, and issue invoices</h3>
          </div>
        ...
        {invoiceError ? <ErrorPill error={invoiceError} /> : null}
        ...
        <div className="space-y-3">
          {orders.length ? (
            orders.map((order) => {
              ...
              const canCreateInvoice = !invoice && (order.status === 'CONFIRMED' || order.status === 'COMPLETED');
                ...
                <article key={order.id} className="rounded-[24px] border border-ink/10 bg-white/85 p-4 shadow-float">
                  <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                    ...
                    <div className="flex flex-col gap-2 lg:items-end">
                      ...
                      {canCreateInvoice ? (
                        <button className="button-chip" disabled={isBusy} onClick={() => onCreateInvoice(order.id)} type="button">
